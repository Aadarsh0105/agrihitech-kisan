...
export function ProductGrid({ products, loading }: { products: Product[] | null; loading?: boolean; emptyLabel?: string; }) {
  if (loading) {
    return (
      <div className="grid grid-cols-2 gap-4 sm:gap-5 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5">
        {Array.from({ length: 10 }).map((_, i) => (
          <div key={i} className=" overflow-hidden rounded-[28px] border border-gray-200 bg-white shadow-sm">
  ...
  if (!products || products.length === 0) {
    return (
      <div className="rounded-[32px] border-2 border-dashed border-primary/20 bg-gradient-to-br from-primary-50 to-white py-24 text-center">
  ...
  return (
    <div className="grid grid-cols-2 gap-4 sm:gap-5 md:grid-cols-3 lg:grid-cols-4 xl:gap-6 2xl:grid-cols-5">
      {products.map((p, i) =>
